...
public class RainEffectGameObject : Base3dGameObject
{
    ...
    public Vector3 AreaSize { get; set; } = new(200f, 250f, 200f);
    ...
    public Vector3 RainDirection { get; set; } = new(0f, -1f, 0f);
    public Vector3 WindDirection { get; set; } = new(0.2f, 0f, 0.08f);
    ...
    public override void Draw(GameTime gameTime, GraphicsDevice graphicsDevice, ICamera3D camera)
    {
        ...
        if (_cameraPositionInitialized)
        {
            ...
            for (var i = 0; i < ActiveDropCount; i++)
            {
        ...
        // Depth state setting
        graphicsDevice.DepthState = new(false); // No depth test, no depth write
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        for (var i = 0; i < ActiveDropCount; i++)
        {
            ...
            var windStrength = MathF.Sin(_animationTime * 1.5f + drop.Position.Z * 0.3f) * 0.2f;
            var windStrengthZ = MathF.Cos(_animationTime * 1.2f + drop.Position.X * 0.3f) * 0.15f;
    ...
    private void UpdateDropDensity()
    {
        ...
        {
            // Spawn new drops
            for (var i = ActiveDropCount; i < target; i++)
            {
