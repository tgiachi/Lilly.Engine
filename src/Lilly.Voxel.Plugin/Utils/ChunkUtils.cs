...
public static class ChunkUtils
{
    ...
    {
        var chunkX = (int)Math.Floor(worldPosition.X / ChunkEntity.Size);
        var chunkY = (int)Math.Floor(worldPosition.Y / ChunkEntity.Height);
        var chunkZ = (int)Math.Floor(worldPosition.Z / ChunkEntity.Size);
    ...
    public static List<Vector3> GetChunksInDirection(Vector3 playerPosition, BlockFace facingDirection, int numberOfChunks)
    {
        ...
        var (offsetX, offsetY, offsetZ) = facingDirection switch
        {
            BlockFace.Back   => (0, 0, -1), // Z-
            BlockFace.Front  => (0, 0, 1),  // Z+
            BlockFace.Right  => (1, 0, 0),  // X+
            BlockFace.Left   => (-1, 0, 0), // X-
            BlockFace.Top    => (0, 1, 0),  // Y+
            BlockFace.Bottom => (0, -1, 0), // Y-
            _                => (0, 0, 0)
        };
    ...
    {
        var localX = worldPosition.X % ChunkEntity.Size;
        var localY = worldPosition.Y % ChunkEntity.Height;
        var localZ = worldPosition.Z % ChunkEntity.Size;

        // Handle negative positions
        if (localX < 0)
        {
            localX += ChunkEntity.Size;
        }

        if (localY < 0)
        {
            localY += ChunkEntity.Height;
        }

        if (localZ < 0)
        {
            localZ += ChunkEntity.Size;
        }
    ...
    public static bool IsPositionInChunk(Vector3 worldPosition, Vector3 chunkPosition)
        => worldPosition.X >= chunkPosition.X &&
           worldPosition.X < chunkPosition.X + ChunkEntity.Size &&
           worldPosition.Y >= chunkPosition.Y &&
           worldPosition.Y < chunkPosition.Y + ChunkEntity.Height &&
           worldPosition.Z >= chunkPosition.Z &&
           worldPosition.Z < chunkPosition.Z + ChunkEntity.Size;
    ...
    public static Vector3 NormalizeToChunkPosition(Vector3 worldPosition)
    {
        ...
        var chunkX = (int)Math.Floor(worldPosition.X / ChunkEntity.Size) * ChunkEntity.Size;
        var chunkY = (int)Math.Floor(worldPosition.Y / ChunkEntity.Height) * ChunkEntity.Height;
        var chunkZ = (int)Math.Floor(worldPosition.Z / ChunkEntity.Size) * ChunkEntity.Size;
