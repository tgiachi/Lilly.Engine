...
using Lilly.Engine.GameObjects.UI.Base;
...
public class ScrollableTextGameObject : BaseGameObject2D, IInputReceiver
{
    ...
    public Rectangle<int> Bounds => new(
    ...
    private void Initialize()
    {
        try
        {
            ...
            {
                _logger.Warning("Font '{FontName}' with size {FontSize} could not be loaded",
                    Theme.FontName, Theme.FontSize);
            }
        }
        catch (Exception ex)
        {
    ...
    public void HandleMouse(MouseState mouseState, GameTime gameTime)
    {
        ...
        if (_isDraggingScrollbar)
        {
            ...
            else
            {
                ...
                if (trackHeight > 0)
                {
                    var deltaY = (float)mousePos.Y - _dragStartY;
    ...
    private Rectangle<float> GetScrollbarThumbBounds()
    {
        ...
        return new Rectangle<float>(
            new Vector2D<float>(trackBounds.Origin.X, thumbPosition),
            new Vector2D<float>(ScrollbarWidth, thumbHeight)
        );
    }
    ...
    private IEnumerable<RenderCommand> DrawScrollbar(Rectangle<float> bounds)
    {
        ...
        yield return DrawRectangle(trackBounds, ScrollBarTrackColor, depth: NextDepth());
        ...
        yield return DrawRectangle(thumbBounds, thumbColor, depth: NextDepth());
    ...
    private List<string> BreakLongWord(string word, DynamicSpriteFont font, int maxWidth)
    {
        ...
        return result.Count > 0 ? result : new List<string> { word };
    }
    ...
    {
        return _maxScrollOffset > 0 ? _scrollOffset / _maxScrollOffset : 0f;
    }
