...
using Lilly.Engine.Lua.Scripting.Loaders;
using Lilly.Engine.Lua.Scripting.Utils;
...
namespace Lilly.Engine.Lua.Scripting.Services;
    ...
    private async Task GenerateLuaMetaFileAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var manualModulesSnapshot = _manualModuleFunctions.ToDictionary(
                kvp => kvp.Key,
                kvp => (IReadOnlyCollection<string>)kvp.Value.Keys.ToArray()
            );
